...
function downloadImage(url: string, outFile: string) {
...
async function main() {
    const saveDir = path.join(__dirname, "../../product-images");

    if (!fs.existsSync(saveDir)) {
        fs.mkdirSync(saveDir, { recursive: true });
    }

    const allItems = [...productList, ...foodList];
    ...
    for (let i = 0; i < allItems.length; i++) {
        ...
        await downloadImage(item.img, filePath);
    }
    ...
    for (let i = 0; i < allItems.length; i++) {
        const item = allItems[i];
        if (!item.img) continue; // bỏ qua nếu không có url ảnh

        const fileName = getFileName(item, i); // dùng index i để tránh trùng
        const filePath = path.join(saveDir, fileName);
    ...
    console.log("🎉 CHECK COMPLETE — All images checked!");
}
